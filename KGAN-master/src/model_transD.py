...
class KGAN(object):
    ...
    def _build_inputs(self):
        ...
        for hop in range(self.n_hop):
            self.memories_h.append(
                tf.placeholder(dtype=tf.int32, shape=[None, 25, self.n_memory], name="memories_h_" +str(hop))
            )
            self.memories_r.append(
                tf.placeholder(dtype=tf.int32, shape=[None, 25, self.n_memory], name="memories_r_" +str(hop))
            )
            self.memories_t.append(
                tf.placeholder(dtype=tf.int32, shape=[None, 25, self.n_memory], name="memories_t_" +str(hop))
            )
    ...
    def _build_embeddings(self):
        ...
        self.relation_emb_matrix = tf.get_variable(name="relation_emb_matrix", dtype=tf.float32,
                                                   ...
                                                   initializer=tf.contrib.layers.xavier_initializer())
        self.ent_transfer = tf.get_variable(name="ent_transfer", dtype=tf.float32,
                                                 shape=[self.n_entity, self.dim],
                                                 initializer=tf.contrib.layers.xavier_initializer())
        self.ent_transfer = tf.concat([self.ent_transfer, last_entity_init], 0)
        self.rel_transfer = tf.get_variable(name="rel_transfer", dtype=tf.float32,
                                            shape=[self.n_relation, self.dim],
                                            initializer=tf.contrib.layers.xavier_initializer())
    ...
    def _build_model(self, args):
        self.transform_matrix = tf.get_variable(name="transform_matrix", shape=[self.dim, self.dim], dtype=tf.float32,
                                                initializer=tf.contrib.layers.xavier_initializer())
        self.oR_matrix = tf.get_variable(name="oR_matrix", shape=[self.dim, self.dim], dtype=tf.float32,
                                                initializer=tf.contrib.layers.xavier_initializer())
        ...
        for i in range(self.n_hop):
            ...
            self.h_t_emb_list.append(tf.nn.embedding_lookup(self.ent_transfer, memories_h))

            self.r_t_emb_list.append(tf.nn.embedding_lookup(self.rel_transfer, memories_r))

            self.t_t_emb_list.append(tf.nn.embedding_lookup(self.ent_transfer, memories_t))

        for hop in range(self.n_hop):
            self.h_transfer_list.append(self._calc(self.h_emb_list[hop], self.h_t_emb_list[hop], self.r_t_emb_list[hop]))
            self.t_transfer_list.append(self._calc(self.t_emb_list[hop], self.t_t_emb_list[hop], self.r_t_emb_list[hop]))
    ...
    def _calc(self, h, t, r):
        ...
        ht = tf.reduce_sum(h * t, axis=2, keep_dims=True)
    ...
    def intra_inter_group_attention(self, args):
        ...
        for hop in range(self.n_hop):
            ...
            Rh = tf.concat([self.h_transfer_list[hop], self.r_emb_list[hop]], axis=2)
            Rh = Dense(
                units=args.dim, activation=None, name='aggregate_Rh', 
                kernel_regularizer = tf.contrib.layers.l2_regularizer(args.l2_weight)
            )(Rh)
            Rh = Dropout(0.5)(Rh)
            Rh = tf.nn.relu(Rh, name='relu_Rh')
            ...
            o = tf.reduce_sum(self.t_transfer_list[hop] * probs_expanded, axis=1)
            ...
            attention = tf.nn.relu(attention, name='relu_attention')
    ...
    def _build_loss(self):
        ...
        self.kge_loss = 0
        for hop in range(self.n_hop):           
            hR = tf.concat([self.h_transfer_list[hop], self.r_emb_list[hop]], axis=2)
            hR = Dense(
                units=self.dim, activation='relu', name='hR_dense', use_bias = False,
                kernel_regularizer = tf.contrib.layers.l2_regularizer(self.l2_weight)
            )(hR)
            hR = Dropout(0.5)(hR)

            hR_expanded = tf.expand_dims(hR, axis=3)
            t_transfer_expanded = tf.expand_dims(self.t_transfer_list[hop], axis=2)

            hRt = tf.squeeze(tf.matmul(hR_expanded, t_transfer_expanded))
        ...
        for hop in range(self.n_hop):
            self.l2_loss += tf.reduce_mean(tf.reduce_sum(self.h_transfer_list[hop] * self.h_transfer_list[hop]))
            self.l2_loss += tf.reduce_mean(tf.reduce_sum(self.t_transfer_list[hop] * self.t_transfer_list[hop]))
